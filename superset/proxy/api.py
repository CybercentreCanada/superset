...
class ProxyRestAPI(BaseSupersetModelRestApi):
    ...
    def get_ipstring(self, id : str, **kwargs: Any) -> Response:
        ...
        if refresh_token:
            ...
            headers["Accept"] = "application/json"
            headers["Authorization"] = f"Bearer { alfred_token }"
            url = "https://alfred-tst.u.chimera.azure.cyber.gc.ca:9488/rest/search/cypher?expression=MATCH%20(ip%3AIP_ADDRESS)%20WHERE%20ip.value%20IN%20%5B%22" + id + "%22%5D%20RETURN%20ip.value%2C%20ip.maliciousness%2C%20ip.creation_date%2C%20ip.created_by%2C%20ip.uri%2C%20ip.report_uri"
            alfred_resp = ""
            try:
                alfred_resp = requests.get(url, headers=headers)
            except Exception as e:
