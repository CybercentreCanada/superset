...
class ChartDataRestApi(ChartRestApi):
    ...
    def data(self) -> Response:
        ...
        if 'extra_form_data' in json_body['form_data'] and 'custom_form_data' in json_body['form_data']['extra_form_data']:
            ...
            temp_col = [cn for cn in sample['colnames'] if str(
                sample['coltypes'][sample['colnames'].index(cn)]) == 'GenericDataType.TEMPORAL']

            if len(temp_col) > 0:
                # we can think about how to decide better
                temp_col = temp_col[0]
                if custom_params['timeSince'] != '':
                    json_body['queries'][0]['filters'].append(
                        {'col': temp_col, 'op': '>=', 'val': [custom_params['timeSince']]})
                if custom_params['timeUntil'] != '':
                    json_body['queries'][0]['filters'].append(
                        {'col': temp_col, 'op': '<=', 'val': [custom_params['timeUntil']]})
        ...
        try:
            ...
            if adv_type_do_sql:
                ...
                for adv_type, adv_filter_val in custom_params['selectors'].items():
                    if adv_type == '' or adv_filter_val == '':
                        break
                    if adv_type in sample['coladvtypes']:
                        adv_cols = [cn for cn in sample['colnames'] if sample['coladvtypes']
                                    [sample['colnames'].index(cn)] == adv_type]
                        list_of_queries = []
                        for col in adv_cols:
                            list_of_queries.append(
                                col + ' = ' + str(adv_filter_val))
                        list_of_big_queries.append(
                            '(' + ' OR '.join(list_of_queries) + ')')

                query_context.queries[0].extras['where'] = ' AND '.join(
                    list_of_big_queries)
