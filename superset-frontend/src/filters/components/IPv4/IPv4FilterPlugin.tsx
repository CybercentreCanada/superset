 ...
 * under the License.
 */
import { t, DataMask, Behavior, QueryObjectFilterClause } from '@superset-ui/core';
import React, { useEffect, useState } from 'react';
import { Slider } from 'src/common/components';
import { PluginFilterIPv4Props } from './types';
import { Styles } from '../common';

const getIPv4ExtraFormData = (
  col: string,
  lower?: number | null,
  upper?: number | null,
) => {
...
export default function IPv4FilterPlugin(props: PluginFilterIPv4Props) {
  const {
    data,
    formData,
    height,
  ...
  const handleAfterChange = (value: [number, number]) => {
    ...
    setValue(value);
    const dataMask = {
      extraFormData: getIPv4ExtraFormData(col, lower, upper),
      currentState: {
        value,
      },
    };
