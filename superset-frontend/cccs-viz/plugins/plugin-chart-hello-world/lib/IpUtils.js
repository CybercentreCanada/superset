...
exports.default = exports.v6 = void 0;

var _bignumber = require("./bignumber.mjs");
...
var BigInteger = _bignumber.BigNumber;
var v4 = (void 0).v4 = {};
var v6 = (void 0).v6 = {};
exports.v6 = v6;
v4.GROUPS = 4;
...
v6.RE_URL_WITH_PORT = new RegExp(/\[([0-9a-f:]+)\]:([0-9]{1,5})/); // Convenience functions
...
function simpleRegularExpression(addressArray) {
  ...
  for (i = 0; i < addressArray.length; i++) {
    ...
    if (segment.length < 4) {
      output.push((0, _sprintfJs.sprintf)('0{0,%d}%s', 4 - segment.length, segment));
    } else {
      output.push(segment);
    }
  }
...
v4.Address.prototype.isCorrect = function () {
  return this.addressMinusSuffix === this.correctForm() && (this.subnetMask === 32 || this.parsedSubnet === String(this.subnet.replace('/')));
};
/*
...
v6.Address = function (address, opt_groups) {
  ...
  if (subnet) {
    ...
    if (isNaN(this.subnetMask) || this.subnetMask < 0 || this.subnetMask > v6.BITS) {
      this.valid = false;
      this.error = "Invalid subnet mask.";
      return;
    }
...
v6.Address.prototype.isCorrect = function () {
  return this.addressMinusSuffix === this.correctForm() && (this.subnetMask === 128 || this.parsedSubnet === String(this.subnet.replace('/')));
};
/*
...
v6.Address.prototype.href = function (opt_port) {
  if (opt_port === undefined) {
    ...
    opt_port = (0, _sprintfJs.sprintf)(':%s', opt_port);
  }

  return (0, _sprintfJs.sprintf)('http://[%s]%s/', this.correctForm(), opt_port);
};
/*
 * Returns the first n bits of the address, defaulting to the
 * subnet mask
...
v6.Address.spanAll = function (s, opt_offset) {
  ...
  return map(letters, function (n, i) {
    return (0, _sprintfJs.sprintf)('<span class="digit value-%s position-%d">%s</span>', n, i + opt_offset, v6.Address.spanAllZeroes(n)); // XXX Use #base-2 .value-0 instead?
  }).join('');
};
/*
...
v6.Address.group = function (addressString) {
  ...
  if (address4) {
    ...
    address6.address = address6.address.replace(v4.RE_ADDRESS, (0, _sprintfJs.sprintf)('<span class="hover-group group-v4 group-6">%s</span>' + '.' + '<span class="hover-group group-v4 group-7">%s</span>', segments.slice(0, 2).join('.'), segments.slice(2, 4).join('.')));
  }
...
v6.Address.prototype.parse = function (address) {
  ...
  if (badCharacters) {
    this.valid = false;
    this.error = (0, _sprintfJs.sprintf)("Bad character%s detected in address: %s", badCharacters.length > 1 ? 's' : '', badCharacters.join(''));
    this.parseError = address.replace(v6.RE_BAD_CHARACTERS, '<span class="parse-error">$1</span>');
    return;
  }
  ...
  if (badAddress) {
    ...
    this.parseError = address.replace(v6.RE_BAD_ADDRESS, '<span class="parse-error">$1</span>');
    return;
  }
  ...
  groups = map(groups, function (g) {
    return (0, _sprintfJs.sprintf)('%x', parseInt(g, 16));
  });
  ...
  groups.forEach(function (group, i) {
    if (groups.length > 4 && !this.v4) {
      this.valid = false;
      this.error = (0, _sprintfJs.sprintf)("Group %d is too long", i + 1);
      return;
    }
  });
  this.valid = true;
...
v6.Address.prototype.regularExpressionString = function (opt_subString) {
  ...
  if (address6.elidedGroups === 0) {
    ...
    output.push('::|'); // TODO: Validate this
    ...
    for (i = 0; i < address6.elidedGroups; i++) {
      ...
      output.push((0, _sprintfJs.sprintf)('(0{1,4}:){%d}:%s', address6.elidedGroups, pipe));
    }
  } else {
    // The elided case
    ...
    output.push((0, _sprintfJs.sprintf)('((0{1,4}:){%d}|:)', address6.elidedGroups));
...
v6.Address.prototype.canonicalForm = function () {
  ...
  return map(this.parsedAddress, function (n) {
    return (0, _sprintfJs.sprintf)("%04x", parseInt(n, 16));
  }).join(':');
};
/*
