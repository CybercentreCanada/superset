...
function transformProps(chartProps) {
  ...
  const columns = datasource == null ? void 0 : datasource.columns;
  console.log('formData via TransformProps.ts', formData);
  const columnMap = new Map();
  columns.reduce(function (columnMap, obj) {
    // columnMap[obj.columnName] = obj;
    return columnMap;
  }, columnMap);
  const formColumns = formData.columns;
  const columnDefs = formColumns.map(c => {
    return {
      field: c,
      minWidth: 50,
